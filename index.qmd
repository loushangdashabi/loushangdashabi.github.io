...
```{python}
#| display-language: true
one_agent_wealth = agent_wealth.xs(8, level="AgentID")

g = sns.lineplot(data=one_agent_wealth, x="Step", y="Wealth")
g.set(title="Wealth of agent 8 over time");
plt.show()
```
```{r}
#| display-language: true
...
agent_wealth %>% 
  ...
  summarise(mean = mean(Wealth), 
            lower = mean(Wealth) - sd(Wealth)/sqrt(n()), 
            upper = mean(Wealth) + sd(Wealth)/sqrt(n())) %>% 
  ggplot()+
