...
py_grid_to_r = function(data){
  data |> 
  as_tibble(.name_repair = "unique_quiet") |> 
  rename_with(~ as.character(seq_along(.))) |> 
  mutate(row = factor(1:n())) |> 
  pivot_longer(-row, names_to="col", values_to="value") |> 
  mutate(col = as.numeric(col) |> factor())
}

agent_counts = py_grid_to_r(py$agent_counts)

ggplot(agent_counts,aes(col, row, fill = value, label = value)) +
  geom_tile() +
  geom_text(color = "white") +
  scale_fill_viridis_c(option = "viridis") + 
  scale_y_discrete(limits = rev(levels(agent_counts$row))) +  
  coord_fixed() + 
  theme_minimal() +
...
tidyheatmap(agent_counts, row, col, value, colors = viridis::viridis(100),
...
```{r}
#| display-language: true
all_wealth_r = tibble(wealth = py$agent_wealth)
...
```{r}
#| display-language: true
agent_counts_r = py_grid_to_r(py$agent_counts)
tidyheatmap(agent_counts_r, row, col, value, colors = viridis::viridis(100),
            display_numbers = T, number_format = "%.0f", number_color = "white",
...
#| display-language: true
model = MoneyModel(100, 10, 10)
#访问 model 中的 grid 属性中的 capacity 属性，修改为 3
model.grid.capacity = 3
